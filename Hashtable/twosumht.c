...
typedef struct ELEMENT {
    int key;    // klucz w tablicy
    int value;  // indeks w tablicy
} element __attribute__((aligned(4)));

// Struktura przechowująca tablicę hash
typedef struct HT {
    int size;       // rozmiar tablicy hash
    element** table; // wskaźnik na tablicę wskaźników elementów
} HT __attribute__((aligned(4)));
...
int main(void) {
    ...
    int* ans = twoSum(nums,size,target,&rs);
